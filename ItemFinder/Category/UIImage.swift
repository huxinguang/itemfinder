...
extension UIImage{
    ...
    static func stretchImage(name:String!) -> UIImage? {
        ...
        if let image = UIImage(named: name)  {
             result = image.resizableImage(withCapInsets: UIEdgeInsets(top: image.size.height * 0.5, left: image.size.height * 0.5, bottom: image.size.height * 0.5, right: image.size.height * 0.5), resizingMode: .tile)
        }else{
            result = nil
        }
        return result
